...
- **⚡ 极致加载速度与首屏秒开优化**：
  - **首屏早期数据并行预取（Early Data Prefetching）**：在 HTML `<head>` 头部内联脚本中与 CSS/JS 资源下载完全并行同步发起 `/api/public`、`/api/me` 与 `/api/nodes` 请求，彻底打破传统 SPA 串行瀑布流，数据与应用几乎同时就绪。
  - **首屏并发直出管线（Parallel Hydration Pipeline）**：优化应用壳层加载依赖，消除权限检查与节点加载之间的串行排队阻断，首屏进入即全速并行直出。
